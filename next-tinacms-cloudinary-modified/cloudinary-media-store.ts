...
export class CloudinaryMediaStore implements MediaStore {
  ...
  async persist(media: MediaUploadOptions[]): Promise<Media[]> {
    ...
    for (const item of media) {
      ...
      // I'm okay with my assets being saved based on the path of the page for now while I wait on
      // https://github.com/tinacms/tinacms/issues/2850 to have better control.
      // This solution may not work for all, feel free to tweak the window.location logic or just hard code a value
      ...
      const parsedRes: Media = {
        ...
        directory: "/",
        previewSrc: fileRes.url, // FIXME: Remove?
        src: fileRes.url,
      };
